...
**Before adding a new hostConfig option, read `/.notes/claude-rulesets/hostConfig-ruleset.md`** — it has the decision tree (boolean vs attribute set vs enum), implementation steps, and gotchas.
...
- `claude-rulesets/hostConfig-ruleset.md` — required reading before adding any hostConfig option
...
- `minecraft_servers/{GTNH,cosmic_frontiers,TerraFirmaGreg-Modern}/` — each pack has `index.md` listing its sub-files (server setup, mods, config tweaks, etc.). **Update the relevant sub-file when that pack's config, mods, or settings change.**
