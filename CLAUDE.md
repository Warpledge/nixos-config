...
- Service toggles: `mullvad.enable`, `clamav.enable`, `docker.enable`, `winboat.enable`, `sunshine.enable`, `cohesion.enable`, `discord.arrpc.enable`, `waydroid.{enable,magisk,nftables}`
...
Desktop and laptop currently differ **only** in `waydroid.enable` and `clamav.enable` (both true on desktop, false on laptop). Everything else is byte-identical apart from the header comment — keep it that way.

Two gotchas: `nautilus` is imported **unconditionally** in `programs/default.nix` despite sitting under `fileBrowsers`, and `gameLaunchers.steam` / `.twintail` are wired in `shared/modules/nixos/programs/gaming/core.nix` while the other four launchers are wired in `home-manager/programs/default.nix`. Grep the option name before assuming which `default.nix` owns it.
